...
## Follow-up portability audit (2026-07-26)

- The current ignored SQLite file passes integrity and foreign-key checks, contains all 15 migration records with none unfinished/rolled back, and is operationally schema-compatible. Three records carry the documented checksums of the earlier destructive migration variants described above; startup now recognizes only those exact legacy checksums, warns about them, and rejects every unknown checksum mismatch, missing applied migration file, or duplicate applied record.
- Reapplying the complete safe migration tree to a test database is covered explicitly: a second `prisma migrate deploy` records and changes nothing. Current committed migrations preserve legacy workflow tables/rows, quantities, row counts, and protected accounting aggregates.
- The incompatible `xcode -> uuid@11` override was removed so a clean `npm ci` produces a valid dependency tree. The high-severity `brace-expansion`, `js-yaml`, and `shell-quote` advisories reported on 2026-07-26 were fixed with compatible locked overrides. The current npm advisory database still reports a moderate `uuid@7` advisory through Expo's iOS-only `xcode@3.0.1` configuration dependency; Expo's compatible dependency graph has no patched `xcode` release, and npm proposes breaking/downgrade fixes. This path is not used by the Linux runtime, backend, exported web POS, or Android build, but remains a dependency-chain risk to revisit on the next Expo upgrade.
- Full dependency verification now runs after clean/bundled installs and before trusting an existing install. Its cache marker includes OS, CPU architecture, libc version, and Node ABI, so copied native modules cannot be accepted on an incompatible computer. Portable packages reject removed workspace residue, TypeScript build-info files, runtime data, private configuration, databases/sidecars/backups, caches, generated builds, and escaping symlinks.
- Browser/device local storage is not restaurant accounting storage. A computer transfer preserves operational information by moving the stopped SQLite file; Android pairing and desktop drag positions must be re-paired/recreated unless separately migrated through platform-specific browser/device tooling.
